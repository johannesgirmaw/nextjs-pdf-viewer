...
export class PdfViewerElement extends HTMLElement {
  ...
  connectedCallback() {
    ...
    searchInput.addEventListener("input", () => {
      // Clear search hits when input changes to allow new search
      this.searchHits = [];
      this.searchIndex = -1;
      void doSearch();
    });
    searchInput.addEventListener("keydown", async (e) => {
      if ((e as KeyboardEvent).key === "Enter") {
        e.preventDefault();
        const q = searchInput.value.trim();
        // If search input has changed or no results yet, perform new search
        if (q !== lastSearchQuery || this.searchHits.length === 0) {
          await doSearchNow();
        } else {
          // Go to next result if search was already performed
    ...
    const onScroll = () => {
      ...
      this.scrollRaf = requestAnimationFrame(() => {
        ...
        const pages = Array.from(
          this.viewerContainer.querySelectorAll(".pdf-page")
        ) as HTMLElement[];
